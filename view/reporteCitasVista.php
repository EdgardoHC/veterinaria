<?php
// view/reporteCitasVista.php
require_once __DIR__ . '/../model/ExpedienteReportesDAO.php'; 

$dao = new ExpedienteReportesDAO();
$vacunas = $dao->listarProximasVacunas(); // Ahora trae datos (incluye historial)
$datos_grafico = $dao->contarCitasPorVacuna(); 
$fechaGeneracion = date('d/m/Y H:i');

$labels = json_encode(array_column($datos_grafico, 'vacuna_nombre'));
...
<head>
    <meta charset="utf-8">
    <title>Vista Previa: Próximas Vacunas</title>
...
<body>
    <div class="container-fluid">
        <div class="btn-toolbar justify-content-end">
            ...
            <a href="../reportes/reporteCitasPDF.php?type=pdf" target="_blank" class="btn btn-danger mr-2">Generar PDF 📄</a>
            <a href="../reportes/reporteCitasExcel.php?type=excel" class="btn btn-success">Descargar Excel 📊</a>
        </div>
        
        <div class="card p-4 report-card">
            <div class="encabezado">
                <h1>🗓️ Agenda de Próximas Vacunas (R4)</h1>
                <p>Generado: **<?= $fechaGeneracion ?>**</p>
            </div>
            
            <div class="chart-container" id="graphContainer">
                <h3 class="text-center mb-3 text-secondary">Distribución de Citas por Tipo de Vacuna</h3>
                <canvas id="chartCitas"></canvas>
            </div>
            
            <h3 class="mt-4 text-secondary">Vacunas Pendientes (Próximas Citas)</h3>
            <table class="table data-table table-sm">
                ...
                <tbody>
                    <?php 
                    // Si el array está vacío, mostrar N/A (Aunque el DAO ya debería traer datos)
                    if (empty($vacunas)) { ?>
                        <tr><td colspan="6" class="text-center">No hay próximas citas de vacunación registradas.</td></tr>
                    <?php } else {
                         foreach ($vacunas as $v): ?>
                            <tr>
                                <td><strong class="text-danger"><?= htmlspecialchars($v['fechaproximavacuna'] ?? 'N/A') ?></strong></td>
                                <td><?= htmlspecialchars(($v['nombre_mascota'] ?? '') . ' ' . ($v['apellido_mascota'] ?? '')) ?></td>
                                <td><?= htmlspecialchars($v['nombre_vacuna'] ?? '---') ?></td>
                                <td><?= htmlspecialchars($v['numerorefuerzo'] ?? '0') ?></td>
                                <td><?= htmlspecialchars($v['nombre_encargado'] ?? '---') ?></td>
                                <td><?= htmlspecialchars($v['telefonomovil'] ?? 'N/D') ?></td>
                            </tr>
                        <?php endforeach; 
                    } ?>
                </tbody>
    ...
    <script>
        ...
        const chartLabelsCitas = <?php echo $labels; ?>;
        const chartDataCitas = <?php echo $data; ?>;
        let myChartCitas;

        function createChartCitas(type) {
            if (myChartCitas) { myChartCitas.destroy(); }
            
            const dataConfig = {
                labels: chartLabelsCitas,
                datasets: [{
                    label: (type === 'bar' ? 'Cantidad de Citas' : 'Distribución'),
                    data: chartDataCitas,
            ...
            myChartCitas = new Chart(ctxCitas, { type: type, data: dataConfig, options: options });
        ...
        window.toggleChartType = function(type) { if (chartLabelsCitas.length > 0) createChartCitas(type); };
        toggleChartType('bar'); 
        ...
        button.addEventListener('click', () => {
            if (graphContainer.style.display === 'none') {
                ...
                toggleChartType(myChartCitas.config.type); 
            } else {
